...
namespace ServiceStack.OrmLite
{
	public class ModelDefinition
	{
	    ...
	    private static readonly Dictionary<Type, ModelDefinition> _definitions = new Dictionary<Type, ModelDefinition>();
        public static ModelDefinition GetModelDefinition(Type type)
        {
            ModelDefinition definition;
            if (!_definitions.TryGetValue(type, out definition))
                definition = _definitions[type] = type.GetModelDefinition();
    ...
    public static class ModelDefinition<T>
    {
		...
		{
			get { return definition ?? (definition = ModelDefinition.GetModelDefinition(typeof(T))); }
		}
